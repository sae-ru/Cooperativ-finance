...
Закрыта последняя frontend-граница хозяйственных величин:

- общий `BigInt` decimal-модуль считает и сравнивает десятичные строки без
  JavaScript `Number`;
- количество, паи, цены, стоимость, ущерб, резервы и покрытие форматируются без
  потери точности;
- округление применяется только к отображению, а не к исходному значению;
- статический regression-тест запрещает возврат бизнес-полей в binary float;
- backend `Decimal` и PostgreSQL `Numeric` подтверждены как неизменная точная
  часть контракта.
