...
## Финальная компенсация Slice 30

| Угроза | Контроль | Остаточный риск |
|---|---|---|
| Жалоба или assessment сразу списывает паи | никакого auto execution; только final decision, отдельная authorization и acceptance | кооператив может утвердить процедурно несправедливый регламент |
| Первоначальный или апелляционный арбитр сам авторизует перенос | запрет всех decision makers по делу, сторон и assessor; отдельный controller | сговор независимых учётных записей требует внешнего надзора |
| Компенсация затрагивает protected amount | service bounds, row locks и DB constraints по доступному `GUARANTEE` остатку | неверный protected amount, ранее утверждённый governance-процессом |
| Гонка создаёт двойной дебет | optimistic version, row locks, active-case unique index и idempotency registry | привилегированный прямой доступ к БД вне приложения |
| Чужой пользователь принимает компенсацию | actor.member_id обязан совпасть с владельцем целевого `PRIMARY` счёта | захваченная персональная сессия до её отзыва |
| Аннулирование скрывает уже принятое возмещение | void только до acceptance, независимый оператор, причина/evidence, append-only event | недобросовестная договорённость операторов до принятия |
| Rollback схемы стирает историю | downgrade `0035 -> 0034` fail-closed при наличии compensation history | аварийное восстановление требует утверждённого ручного плана |
| Технический transfer объявляется юридическим взысканием | UI/docs явно ограничивают baseline и запрещают auto-claim | правовая действительность зависит от юрисдикции и устава кооператива |

## Точная десятичная граница Slice 31

| Угроза | Контроль | Остаточный риск |
|---|---|---|
| браузер округляет большой паевой остаток | API decimal string разбирается в `BigInt` coefficient/scale | сторонний экран может обойти общий helper |
| сумма `0.1 + 0.2` меняет видимый итог | все бизнес-суммы складываются и вычитаются decimal helper | ошибка в новом алгоритме требует независимого review |
| сортировка выбирает неверный предел компенсации | сравнение и минимум выполняются на выровненных целых коэффициентах | сервер остаётся окончательным enforcement boundary |
