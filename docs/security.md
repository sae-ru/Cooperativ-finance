...
Алгоритм `2.0.0` выполняет межконтурные проверки server-side и сохраняет в
сигнале только хозяйственные UUID, количества, интервалы, агрегаты и применённые
пороги. Имена, логины, телефоны, адреса, приватные evidence payload и текст
нуждаемости в observed data не копируются. Rule catalog статичен и не содержит
данных конкретного кооператива.

Доступ к scans/signals остаётся role/scoped; сигнал не создаёт публичный risk
score. Для связанных лиц, санкций, солидарной помощи и репутации до пилота нужен
отдельный privacy/legal review: техническая минимизация не заменяет законное
основание обработки и процедуру исправления ошибочной связи.
